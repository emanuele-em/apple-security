...
#[cfg(target_os = "ios")]
use apple_security_sys::base::{errSecNotTrusted, errSecSuccess};
use apple_security_sys::base::{errSecParam, SecCertificateRef};
use apple_security_sys::certificate::*;
use apple_security_sys::keychain_item::SecItemDelete;
use std::fmt;
use std::ptr;
...
use apple_security_sys::item::kSecValueRef;
#[cfg(any(feature = "OSX_10_12", target_os = "ios"))]
use std::ops::Deref;
...
impl SecCertificate {
    ...
    fn pk_to_der(&self, public_key: key::SecKey) -> Option<Vec<u8>> {
        use apple_security_sys::item::kSecAttrKeyType;
        use apple_security_sys::item::kSecAttrKeySizeInBits;
...
fn get_asn1_header_bytes(pkt: CFString, ksz: u32) -> Option<&'static [u8]> {
    use apple_security_sys::item::kSecAttrKeyTypeRSA;
    use apple_security_sys::item::kSecAttrKeyTypeECSECPrimeRandom;
